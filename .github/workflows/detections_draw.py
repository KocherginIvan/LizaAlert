...
def all_colors(promt_list):
  ...
  class_color_1 = {}
  j = np.random.randint(len(all_colors))
  a = 0
  for i in promt_list:
    if j >= len(all_colors):
      a = 1
...
def display_images_grid(images,
  ...
  img_bbox_list = []
  n_images = len(images)
  #rows_number = math.ceil(n_images/imagesNumber_per_row)
  plt.figure(figsize=(figsize_ratio[0], figsize_ratio[1]), facecolor=facecolor)
  for i in range(n_images):
      plt.figure(figsize=(figsize_ratio[0], figsize_ratio[1]), facecolor=facecolor)
      #plt.subplot(rows_number, imagesNumber_per_row, i+1)
      img = images[i]
...
def draw_test(img_list, boxes_list, phrase_list, phrases_2, logits, facecolor, promter):   
   _, promt_list, _ = config.promter(promter)
